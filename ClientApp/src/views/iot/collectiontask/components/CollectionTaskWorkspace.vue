<template>
	<div class="collection-task-page" :class="{ 'collection-task-page--embedded': embedded }">
		<div class="collection-task-page__content">
			<div class="collection-task-page__filters">
				...
				<el-input v-if="!isDeviceScoped" v-model="query.gateway" placeholder="网关设备ID" clearable @keyup.enter="handleSearch" />
				...
				<el-button type="success" @click="openCreateDialog">{{ createButtonText }}</el-button>
			</div>
			...
			<el-form ref="formRef" :model="form" :rules="formRules" label-width="120px">
				<el-tabs v-model="activeEditorTab">
						...
						<el-row :gutter="16">
							...
							<el-col :span="12">
								<el-form-item label="网关设备ID" prop="edgeNodeId">
									<el-input v-model="form.edgeNodeId" :disabled="isDeviceScoped" placeholder="输入网关 DeviceId" />
								</el-form-item>
							</el-col>
						</el-row>
						<el-row :gutter="16">
					...
					<el-tab-pane label="点位配置" name="points">
						...
						<el-table :data="selectedPoints" row-key="_rowId" class="editor-table point-editor-table" empty-text="暂无点位">
...
const isDeviceScoped = computed(() => Boolean(props.deviceId));
const createButtonText = computed(() => (isDeviceScoped.value ? '新增当前设备任务' : '新增任务'));
...
const filteredRows = computed(() => {
	return state.tableData.rows.filter((item) => {
		...
		const matchesDevice = !props.deviceId || item.edgeNodeId === props.deviceId;
		return matchesTask && matchesGateway && matchesProtocol && matchesEnabled && matchesDevice;
	});
});
...
function toPayload() {
	const payload = {
		...form,
		edgeNodeId: props.deviceId || form.edgeNodeId,
		connection: {
			...form.connection,
...
function openEditDialog(row: any) {
	...
	Object.assign(form, {
		...
		edgeNodeId: props.deviceId || row.edgeNodeId || '',
		connection: {
...
.collection-task-page--embedded .collection-task-page__content {
	padding: 0;
	border: 0;
